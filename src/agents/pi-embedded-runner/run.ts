...
export async function runEmbeddedPiAgent(
  ...
  log.debug(
    `trace run enqueue: runId=${params.runId} sessionLane=${sessionLane} globalLane=${globalLane}`,
    {
      ...runTrace,
      enqueuedAt,
    },
  ...
  return enqueueSession(
    () =>
      enqueueGlobal(
        ...
        {
          warnAfterMs: 2_000,
          onStart: (waitMs, queuedAhead) => {
            log.debug(
              `trace lane acquired: runId=${params.runId} lane=${globalLane} waitMs=${waitMs} queuedAhead=${queuedAhead}`,
              {
                ...runTrace,
                lane: globalLane,
                scope: "global",
      ...
      ),
    {
      ...
      onStart: (waitMs, queuedAhead) => {
        log.debug(
          `trace lane acquired: runId=${params.runId} lane=${sessionLane} waitMs=${waitMs} queuedAhead=${queuedAhead}`,
          {
            ...runTrace,
            lane: sessionLane,
            scope: "session",
