...
type NodesRunNodeInfo = {
  platform: string | null;
  commands: string[];
};

async function resolveNodesRunNodeInfo(
  opts: NodesRpcOpts,
  nodeId: string,
): Promise<NodesRunNodeInfo> {
  try {
    ...
    return {
      ...
      commands: Array.isArray(match?.commands)
        ? match.commands.filter((command): command is string => typeof command === "string")
        : [],
    };
  } catch {
    return { platform: null, commands: [] };
  }
}
...
async function prepareNodesRunContext(params: {
  ...
  if (params.raw) {
    ...
    argv = buildNodeShellCommand(rawCommand, params.nodePlatform ?? undefined);
  }
...
function resolveFallbackNodeApprovals(params: { security: ExecSecurity; ask: ExecAsk }) {
  return {
    ...
    nodes
      ...
      .action(async (command: string[], opts: NodesRunOpts) => {
        await runNodesCommand("run", async () => {
          ...
          const nodeInfo = await resolveNodesRunNodeInfo(opts, nodeId);
          ...
          const approvals = nodeInfo.commands.includes("system.execApprovals.get")
            ? await resolveNodeApprovals({
                opts,
                nodeId,
                agentId,
